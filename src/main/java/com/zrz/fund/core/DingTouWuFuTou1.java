...
public class DingTouWuFuTou1 {
	...
	public static List<ExcelPO> dingTou(String start, String end,
    	...
    	listRs.add("allDays\tstartDate\tdate\tclose0\tyear0\tback\tsum\tfloatC\tinput\trate\tyearRate\tall\tinput_all\tmake_all\tsurplus");
    	double count = 0;
    	double sum = 0;
    	double floatC = 0;
    	double input = 0;
    	...
    	for(int i=0;i<listAll.size();i++){
			...
			lastDate = date0;
			//叠加利息
			double make_oneDay = surplus*0.06/365;
...
//    	    				+surplus);
    		...
    		if(week==1 && close0<=year0){
    			count = count + (input_money/close0);
    			input += input_money;
    			input_all +=  input_money;
